...
async def стрелять(ctx):
	emb = discord.Embed(title = random.choice(popal), colour = discord.Color.green())

	emb.set_author(name = bot.user.name, icon_url = bot.user.avatar_url)
	emb.set_footer(text = ctx.author.name, icon_url = ctx.author.avatar_url)
	emb.set_image(url = random.choice(popal1))
...
async def граната(ctx):
	emb = discord.Embed(title = random.choice(granata), colour = discord.Color.green())
...
async def снайпер(ctx):
	emb = discord.Embed(title = random.choice(snaiper), colour = discord.Color.green())
